...
function buildMarkerSheet(result) {
  ...
  aoa.push([
    ...
    '', '', '',
    ...totalProduced,
    totalProduced.reduce((a, b) => a + b, 0),
  ]);

  const ws = XLSX.utils.aoa_to_sheet(aoa);

  // Column widths: Marker(10) + n×ratio(9) + ratioSum(10) + ply(8) + times(8) + n×pieces(12) + total(13)
  setColWidths(ws, [10, ...Array(n).fill(9), 10, 8, 8, ...Array(n).fill(12), 13]);
